...
export class CompanyLegitimacyService {
  ...
  async verifyCompany(companyData: {
    ...
    try {
      ...
      this.logger.error(
        `Error verifying company: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
   ...
   */
  private async checkDomainLegitimacy(
    companyName: string,
    websiteUrl: string,
  ): Promise<number> {
   ...
   */
  private async analyzeCompanyReviews(
    companyName: string,
    location?: string,
  ): Promise<number> {
    // In a real implementation, we would:
   ...
   */
  private async checkCompanyRegistration(
    companyName: string,
    location?: string,
  ): Promise<number> {
    // In a real implementation, we would:
  ...
  private generateRecommendations(scores: {
    ...
    if (scores.overall < 70) {
      recommendations.push(
        'Conduct additional research before proceeding with this company.',
      );
    }
    ...
    if (scores.socialMedia < 50) {
      recommendations.push(
        "Check the company's presence on professional platforms like LinkedIn.",
      );
    }

    if (scores.reviews < 50) {
      recommendations.push(
        'Look for employee reviews on platforms like Glassdoor or Indeed.',
      );
    }

    if (scores.registration < 70) {
      recommendations.push(
        "Verify the company's business registration with relevant authorities.",
      );
    }
  ...
  private getDomainVerificationNotes(score: number): string {
    if (score >= 80)
      return 'The company has a well-established online presence with a legitimate domain.';
    if (score >= 50)
      return 'The company appears to have a legitimate domain, but further verification is recommended.';
  ...
  private getSocialMediaNotes(score: number): string {
    if (score >= 80)
      return 'The company has strong social media presence across multiple platforms.';
    if (score >= 50)
      return 'The company has some social media presence, but activity is limited.';
    return 'The company has minimal or no verifiable social media presence.';
  }

  private getReviewsNotes(score: number): string {
    if (score >= 80)
      return 'The company has positive and consistent reviews from employees and customers.';
    if (score >= 50)
      return 'The company has mixed reviews or limited review history.';
    return 'The company has negative reviews or insufficient review data.';
  }

  private getRegistrationNotes(score: number): string {
    if (score >= 80)
      return 'The company is properly registered and in good standing.';
    if (score >= 50)
      return 'The company appears to be registered, but some details could not be verified.';
  ...
  private getWebsiteNotes(score: number): string {
    if (score >= 80)
      return 'The company website is professional, secure, and contains comprehensive information.';
    if (score >= 50)
      return 'The company website is functional but may lack some important information.';
  ...
  private simulateRegistrationCheck(
    companyName: string,
    location?: string,
  ): number {
