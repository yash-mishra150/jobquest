...
export class CompanyLegitimacyController {
  ...
  constructor(private readonly companyLegitimacyService: CompanyLegitimacyService) {}

  @Post('verify')
  async verifyCompany(@Body() companyData: {
    name: string;
    location?: string;
    about?: string;
  }) {
    ...
    try {
      ...
      const result = await this.companyLegitimacyService.verifyCompany(companyData);
      return result;
    } catch (error) {
      this.logger.error(`Error during company verification: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  async checkCompany(
    ...
    try {
      ...
      this.logger.error(`Error during company check: ${error.message}`, error.stack);
      return {
        success: false,
