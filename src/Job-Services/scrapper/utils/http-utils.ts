...
export async function fetchWithRetry(url: string, options: any = {}, maxRetries = 3): Promise<any> {
  let retries = 0;
  let lastError;
...
export function extractAttr($element: cheerio.Cheerio<any> | null, attr: string): string {
  if (!$element || $element.length === 0) return '';
