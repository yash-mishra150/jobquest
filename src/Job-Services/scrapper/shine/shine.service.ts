...
export class ShineService {
  ...
  async getJobs(queryOptions: Record<string, any>): Promise<JobResponseDto> {
    ...
    try {
      ...
      const allJobs: JobDto[] = result.jobs;
      ...
      console.log(
        `Found ${allJobs.length} total jobs (Shine reports ${totalJobsReported} total)`,
      );
      ...
      if (startIndex >= totalItems) {
        console.log(
          `Page ${page} exceeds available results (${totalItems} total items)`,
        );
        return {
          success: true,
      ...
      console.log(
        `Returning ${pageItems.length} jobs for page ${page} (items ${startIndex + 1}-${endIndex})`,
      );
      ...
      if (page === 1 || queryOptions.fetchDetails === true) {
        console.log(
          `Fetching detailed information for ${pageItems.length} jobs on page ${page} (this may take a moment)...`,
        );
        finalResults = await this.fetchDetailsForJobs(pageItems);
      } else {
        console.log(
          `Skipping detailed information for page ${page} to improve performance`,
        );
        ...
        finalResults = pageItems.map(item => ({
          ...
          jobDescription:
            'Request with fetchDetails=true parameter to get complete information',
  ...
  private async fetchAllJobs(url: string): Promise<any> {
    ...
    try {
      ...
      if (axiosResult && axiosResult.jobs && axiosResult.jobs.length > 0) {
        console.log(
          `Successfully fetched ${axiosResult.jobs.length} jobs with Axios`,
        );
        return axiosResult;
      }
  ...
  private async fetchAllJobsWithAxios(url: string): Promise<any> {
    try {
      ...
      try {
        const totalText = $(
          '.jsrpLeftSection_found__dWFE0 span.jsrpLeftSection_foundHeading__IiD_O',
        ).text();
      ...
      $('.jobCardNova_bigCard__W2xn3').each((_, element) => {
        try {
          ...
          const companyName = $(element)
            .find('.jobCardNova_bigCardTopTitleName__M_W_m')
            .text()
            .trim();

          // Get location
          const location = $(element)
            .find('.jobCardNova_limitsLocation__eHDH7')
            .text()
            .trim();

          // Get experience
          const experience = $(element)
            .find('.jobCardNova_bigCardCenterListExp__KTSEc')
            .text()
            .trim();

          // Get posted date
          const postedDate = $(element)
            .find('.jobCardNova_postedData__LTERc')
            .text()
            .trim();
          ...
          const skills: string[] = [];
          $(element)
            .find('.jobCardNova_skillsLists__7YifX li')
            .each((_, skillElement) => {
          ...
          const jobType = $(element)
            .find('.jobTypeTagNova_commonTag__WsV1E')
            .text()
            .trim();
  ...
  private async _fetchAllJobsWithPuppeteer(url: string): Promise<any> {
    ...
    try {
      ...
      await page
        .waitForSelector('.jobCardNova_bigCard__W2xn3', { timeout: 8000 })
        .catch(() => {
      ...
      for (let i = 0; i < maxScrolls; i++) {
        ...
        const currentCount = await page.evaluate(() => {
          return document.querySelectorAll('.jobCardNova_bigCard__W2xn3')
            .length;
        });
        ...
        if (currentCount === lastCount) {
          console.log(
            `No new items after scroll, stopping at ${currentCount} items`,
          );
          break;
        }
      ...
      const jobs = await page.evaluate(() => {
        const items = Array.from(
          document.querySelectorAll('.jobCardNova_bigCard__W2xn3'),
        );
        ...
        const getHref = (el: Element, selector: string) => {
          const selected = el.querySelector(selector);
          const href = selected ? selected.getAttribute('href') : null;
          return href
            ? href.startsWith('http')
              ? href
              : 'https://www.shine.com' + href
            : 'Not available';
        };
        ...
        return items.map(el => ({
          ...
          skills: Array.from(
            el.querySelectorAll('.jobCardNova_skillsLists__7YifX li'),
          )
      ...
      try {
        ...
        try {
          ...
          if (details && Object.keys(details).length > 0) {
            ...
            if (
              aboutCompanyInfo === 'Not available' ||
              aboutCompanyInfo === 'Not mentioned'
            ) {
              // Build alternative description from industry and recruiter details
              const industryInfo = details.industry
                ? `Industry: ${details.industry}`
                : '';
              const recruiterInfo = details.recruiterName
                ? `Recruiter: ${details.recruiterName}`
                : '';
              const departmentInfo = details.department
                ? `Department: ${details.department}`
                : '';
              const jobTypeInfo = details.jobType
          ...
          console.log(
            `Axios approach failed for ${item.title}, falling back to Puppeteer`,
          );
        }
  ...
  private async scrapeJobDetailsWithAxios(url: string): Promise<any> {
    try {
      ...
      const positions =
        getText(
          '.jobdetailsNova_jdKeyHighlightList__nAcEl li:nth-child(1)',
        ).replace(/[^0-9]/g, '') || 'Not specified';
      const experience = getText(
        '.jobdetailsNova_jdKeyHighlightList__nAcEl li:nth-child(2)',
      );
      const salary = getText(
        '.jobdetailsNova_jdKeyHighlightList__nAcEl li:nth-child(3)',
      );
      ...
      try {
        ...
        if (!location) {
          const fourthItem = $(
            '.jobdetailsNova_jdKeyHighlightList__nAcEl li:nth-child(4)',
          );
      ...
      $('.jobdetailsNova_jdOtherDetailsLists__iQ49f li').each((_, element) => {
        const label = $(element)
          .find('.jobdetailsNova_jdRole__xh4DN')
          .text()
          .trim()
          .toLowerCase()
          .replace(/\s+/g, '_');
      ...
      const department =
        otherDetails['department'] ||
        getText(
          '.jobdetailsNova_jdOtherDetailsLists__iQ49f li:nth-child(1) strong',
        );
      const industry =
        otherDetails['industry'] ||
        getText(
          '.jobdetailsNova_jdOtherDetailsLists__iQ49f li:nth-child(2) strong',
        );
      const education = otherDetails['education'] || 'Not specified';
      const recruiterDetails =
        otherDetails['recruiter_details'] ||
        getText(
          '.jobdetailsNova_jdOtherDetailsLists__iQ49f li:nth-child(4) strong',
        );
      const jobTags = otherDetails['job_tags'] || 'Not specified';
      const jobType =
        otherDetails['job_type'] ||
        getText(
          '.jobdetailsNova_jdOtherDetailsLists__iQ49f li:nth-child(6) strong',
        );
      ...
      const aboutCompany = getText(
        '.jdAboutCompanyNova_jdAboutCompanyInfo___yGSG',
      );

      // Extract recruiter info
      const recruiterName = getText(
        '.aboutRecruiterNova_jbAboutRecruiterHeading__dijid',
      );
      const recruiterLocation = getText(
        '.aboutRecruiterNova_jbAboutRecruiterSubHeading__4zAS_',
      );
  ...
  private async scrapeJobDetails(url: string): Promise<any> {
    ...
    try {
      ...
      await page
        .waitForSelector('.jobdetailsNova_jobdetails__j8y_W', {
          timeout: 10000,
        })
        .catch(() => {
          // Increased timeout
          console.log(
            'Could not find job details section, returning default values',
          );
        });
      ...
      const results = await page.evaluate(() => {
        ...
        try {
          ...
          const locationDiv = document.querySelector(
            '.jobdetailsNova_jDLocationTxt___U_GN',
          );
          if (locationDiv) {
          ...
          if (!location) {
            const locationLink = document.querySelector(
              '.jobdetailsNova_jDLocationTxt___U_GN a',
            );
            if (locationLink) {
          ...
          location =
            document
              ...
              ?.textContent?.trim() || 'Not mentioned';
        }
        ...
        // Extract other details - more comprehensive approach to get all available details
        const otherDetails: Record<string, string> = {};
        const otherDetailLabels = Array.from(
          document.querySelectorAll(
            '.jobdetailsNova_jdOtherDetailsLists__iQ49f li .jobdetailsNova_jdRole__xh4DN',
          ),
        );
        ...
        otherDetailLabels.forEach(label => {
          ...
          if (labelText && label.parentElement) {
            ...
            if (valueElement) {
              otherDetails[labelText.toLowerCase().replace(/\s+/g, '_')] =
                valueElement.textContent?.trim() || 'Not mentioned';
            }
          }
        });
        ...
        const skills = Array.from(
          document.querySelectorAll(
            '.jdSkillsNova_jdSkillsCardTopList__GU29K li a',
          ),
        ).map(skill => (skill.textContent ? skill.textContent.trim() : ''));
      ...
      console.log(
        `Successfully scraped details for job with ${results.skills ? results.skills.length : 0} skills`,
      );
      return results;
    } catch (err: any) {
  ...
  private cleanJobDescription(text: string): any {
    ...
    const summary =
      cleanedText.substring(0, 150) + (cleanedText.length > 150 ? '...' : '');
    ...
    const responsibilities = text.match(
      /Key Responsibilities:[\s\S]*?(?=Required Skills|Education|Why Join|$)/i,
    );
    if (responsibilities) {
      sections.responsibilities = responsibilities[0]
        .replace('Key Responsibilities:', '')
        .trim();
    }

    const skills = text.match(
      /Required Skills[\s\S]*?(?=Education|Why Join|$)/i,
    );
    if (skills) {
