...
import { fetchWithRetry, parseHTML, extractText, extractAttr } from '../utils/http-utils';
...
export class TimesjobService {
  ...
  private readonly baseUrl = 'https://m.timesjobs.com/mobile/jobs-search-result.html';
  ...
  async getJobs(queryOptions: any): Promise<any> {
    try {
      this.logger.log(`Fetching TimesJobs with options: ${JSON.stringify(queryOptions)}`);
      ...
      const links = await this.getJobLinks(url);
      ...
      if (isTestRequest) {
        this.logger.log(`Test request: Found ${links.length} total TimesJobs jobs`);
        return {
          success: true,
      ...
      this.logger.log(`Processing ${paginatedLinks.length} TimesJobs listings for page ${page}...`);
      ...
      const jobPromises = paginatedLinks.map((link, index) => {
        return Promise.race([
          this.getJobDetails(link).catch(e => {
            this.logger.error(`Error getting job details for ${link}: ${e.message}`);
            return this.getFallbackJob(link);
          }),
          new Promise<any>(resolve => setTimeout(() => {
            this.logger.warn(`Job ${index + 1} (${link}) timed out after ${JOB_TIMEOUT/1000}s`);
      ...
      let allJobDetails = await Promise.all(jobPromises);
      
      // Filter out null results and ensure we have valid jobs
      allJobDetails = allJobDetails.filter(job => job !== null);
      this.logger.log(`Successfully processed ${allJobDetails.length} TimesJobs details`);
      ...
      this.logger.error(`Error scraping TimesJobs: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  private generateUrl(queryOptions: any): string {
    ...
    if (queryOptions.experience) {
      ...
      const expValue = expMap[queryOptions.experience] || queryOptions.experience;
  ...
  private async getJobLinks(url: string): Promise<string[]> {
    ...
    try {
      ...
      this.logger.warn('Failed to get TimesJobs links with Axios, falling back to Puppeteer');
  ...
  private async getJobLinksWithAxios(url: string): Promise<string[]> {
    try {
      ...
      $('.srp-job-bx .srp-job-heading h3 a').each((_, element) => {
        ...
        if (href) {
          ...
          const fullUrl = href.startsWith('http') ? 
            href : `https://m.timesjobs.com${href.startsWith('/') ? '' : '/'}${href}`;
          links.push(fullUrl);
        }
      });
      ...
      this.logger.error(`Error getting TimesJobs links with Axios: ${error.message}`);
  ...
  private async getJobLinksWithPuppeteer(url: string): Promise<string[]> {
    ...
    try {
      ...
      await page.waitForSelector('.srp-job-bx', { timeout: 15000 })
        .catch(() => this.logger.warn('Could not find TimesJobs listing elements'));
      
      // Extract job links
      const links = await page.evaluate(() => {
        const linkElements = document.querySelectorAll('.srp-job-bx .srp-job-heading h3 a');
        return Array.from(linkElements).map(el => {
          const href = el.getAttribute('href');
          if (!href) return "";
          
          // Make sure we have the full URL
          return href.startsWith('http') ? href : `https://m.timesjobs.com${href.startsWith('/') ? '' : '/'}${href}`;
        }).filter(link => link !== "");
      });
      ...
      this.logger.error(`Error getting TimesJobs links with Puppeteer: ${error.message}`);
      await browser.close();
  ...
  private async getJobDetails(url: string): Promise<any> {
    ...
    try {
      ...
      this.logger.warn(`Failed to get TimesJobs details with Axios for ${url}, falling back to Puppeteer`);
  ...
  private async getJobDetailsWithAxios(url: string): Promise<any> {
    try {
      ...
      const location = extractText($('.srp-job-bx-new .srp-loc')).replace('Location:', '').trim();
      ...
      const jobDetails = {
        ...
        experience: this.cleanExperienceText(experience) || 'Experience Not Specified',
        salary: salary || 'Salary Not Specified',
        skills: skills.length > 0 ? skills : ['Skills Not Specified'],
        jobDescription: this.cleanDescription(jobDescription) || 'No Description Available',
        aboutCompany: this.cleanCompanyText(aboutCompany) || 'Company Information Not Available',
      ...
      this.logger.error(`Error getting TimesJobs details with Axios: ${error.message}`);
      throw error; // Rethrow to trigger Puppeteer fallback
    }
  }
   ...
   */
  private async getJobDetailsWithPuppeteer(url: string): Promise<any> {
    ...
    try {
      ...
      ]).catch(() => this.logger.warn('Could not find TimesJobs detail elements'));
      ...
      const jobDetails = await page.evaluate((jobUrl: string) => {
        ...
        return {
          ...
          link: jobUrl,
          companyName: getText('.srp-job-bx-new h2 span:first-child') || 'Company Not Listed',
          location: getText('.srp-job-bx-new .srp-loc').replace('Location:', '').trim() || 'Location Not Specified',
          experience: getText('.srp-job-bx-new .srp-exp') || 'Experience Not Specified',
          salary: getText('.srp-job-bx-new .srp-sal') || 'Salary Not Specified',
          skills: skills.length > 0 ? skills : ['Skills Not Specified'],
          jobDescription: getText('#JobDescription p') || 'No Description Available',
          aboutCompany: getText('#About .comp-detail') || 'Company Information Not Available',
          postedDate: getText('.srp-job-bx-new h2 .posting-time') || 'Date Not Specified',
          source: 'timesjobs',
          timestamp: new Date().toISOString()
        };
      }, url);
      ...
      this.logger.error(`Error getting TimesJobs details with Puppeteer: ${error.message}`);
      await browser.close();
