...
export class NaukriService {
   ...
   */  async getJobs(queryoptions: any): Promise<any> {
    ...
    try {
      ...
      console.log(`Processing ${paginatedLinks.length} job listings for page ${page}...`);
      ...
      console.log(`Processing all ${paginatedLinks.length} jobs in parallel with timeout protection`);
      ...
      const jobPromises = paginatedLinks.map((link, index) => {
        return new Promise(async (resolve) => {
          ...
          const timeoutPromise = new Promise(timeoutResolve => {
            setTimeout(() => {
              console.warn(`Job ${index + 1} (${link}) timed out after ${JOB_TIMEOUT/1000}s`);
          ...
          try {
            ...
            if (jobDetails) {
              if (jobDetails.timedOut) {
                ...
                console.log(`Successfully extracted details for: ${jobDetails.title}`);
              }
              resolve(jobDetails);
            } else {
      ...
      console.log(`Successfully processed ${allJobDetails.length} job details from ${paginatedLinks.length} links`);
  ...
  private async getLinks(url: string): Promise<string[]> {
    ...
    try {
      ...
      if (axiosLinks && axiosLinks.length > 0) {
        console.log(`Successfully fetched ${axiosLinks.length} job links with Axios`);
        return axiosLinks;
      }
    ...
    try {
      ...
      if (links.length === 0) {
        console.log("No links found with specific selectors, trying generic approach");
        $('a').each((_, element) => {
          const href = $(element).attr('href');
  ...
  private async getJobDetails(url: string): Promise<any> {
    ...
    try {
      ...
      if (axiosDetails && axiosDetails.title && axiosDetails.title !== 'Not available') {
        console.log(`Successfully fetched details for job with Axios: ${axiosDetails.title}`);
        return axiosDetails;
      }
      
      console.log('Axios approach failed or returned incomplete results, falling back to Puppeteer...');
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        ]).catch(() => console.log("Could not find primary job detail selectors, continuing anyway"));
      } catch (error) {
    ...
    try {
      ...
      return {
        ...
        jobDescription,
        aboutCompany: aboutCompany !== 'Not available' ? aboutCompany : `Company: ${companyName}`,
        education,
        postedDate,
        timestamp: new Date().toISOString()
      };
  ...
  private cleanJobDescription(text: string): string {
    ...
    for (const section of sections) {
      ...
      const regex = new RegExp(`(${section}:|${section}\\s*:|${section}\\s*-|${section}\\s*–)`, 'gi');
